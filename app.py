...
def scan():
    ...
    Not available in serverless (Vercel) environments.
    """
    if IS_SERVERLESS:
        return jsonify({"success": False, "error": "Port scanning is not available in serverless hosting."}), 503
    ...
    for port in ports:
        ...
        try:
            ...
            results.append({"port": port, "name": get_port_name(port), "status": "open" if res == 0 else "closed"})
        except Exception:
            results.append({"port": port, "name": get_port_name(port), "status": "closed"})
        finally:
            s.close()

    return jsonify({"success": True, "ip": ip, "ports": results})
...
def traceroute():
    ...
    """
    if IS_SERVERLESS:
        return jsonify({"success": False, "error": "Traceroute is not available in serverless hosting."}), 503
        ...
        try:
            ...
            for line in iter(process.stdout.readline, ''):
                ...
                if win_match:
                    ...
                    data = {"hop": hop_num, "rtt": [win_match.group(2).strip(), win_match.group(3).strip(), win_match.group(4).strip()], "ip": "" if is_timeout else ip_part, "timeout": is_timeout}
                    yield f"data: {json.dumps(data)}\n\n"
                elif lin_match:
                    hop_num = int(lin_match.group(1))
                    ip_part = lin_match.group(2).strip()
                    is_timeout = ip_part == "*" or "timeout" in ip_part.lower()
                    data = {"hop": hop_num, "rtt": [lin_match.group(3).strip(), lin_match.group(4).strip(), lin_match.group(5).strip()], "ip": "" if is_timeout else ip_part, "timeout": is_timeout}
                    yield f"data: {json.dumps(data)}\n\n"
